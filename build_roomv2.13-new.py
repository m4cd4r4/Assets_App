...
def update_count(operation):
    ...
    input_value = int(entry_value.get()) if entry_value.get().isdigit() else 0
    if input_value <= 0:
        return  # Early exit if the input value is not valid

    # Initialize successful_operations at the start
    ...
    if "Headset" in selected_item:
        for _ in range(input_value):
            while True:
                ...
                if operation == 'add':
                    existing_serials = [row[0] for row in headsets_sheet.iter_rows(min_row=2, values_only=True)]
                    if serial_number in existing_serials:
                        tk.messagebox.showwarning("Duplicate Serial Number", "This serial number already exists. Please enter a different number.")
                        continue  # Re-prompt for serial number input
                    else:
                        ...
                        successful_operations += 1  # Counting successful operations
                        break
                
                elif operation == 'subtract':
                    found = False
                    for row in headsets_sheet.iter_rows(min_row=2, values_only=False):
                        if row[0].value == serial_number:
                            headsets_sheet.delete_rows(row[0].row)
                            found = True
                            successful_operations += 1  # Counting successful operations
                            break
                    if not found:
                        tk.messagebox.showwarning("Serial Number Not Found", "This serial number does not exist. Please enter a valid number.")
                        continue
                    break

    elif any(g in selected_item for g in ["G8", "G9", "G10"]):
        for _ in range(input_value):
            san_number = show_san_input()
            if san_number is None:  # User cancelled the input
                break  # Exit if SAN input is cancelled
            successful_operations += 1
        input_value = successful_operations

    # Common logic for updating item counts
    item_sheet = workbook[current_sheets[0]]
    for row in item_sheet.iter_rows(min_row=2, values_only=False):
        if row[0].value == selected_item:
            current_count = row[2].value or 0
            if operation == 'add':
                row[2].value = current_count + input_value
            elif operation == 'subtract':
                row[2].value = max(current_count - input_value, 0)
            row[1].value = current_count

    # Log the change only if operations were successful (applicable for all items, not just headsets)
    if successful_operations > 0 or (operation == 'add' and "Headset" in selected_item):

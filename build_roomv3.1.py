# Back to basics
# Removed Serial #, ServiceNow #  Notes logic
                ...
                while entered_sans_count < input_value:
                    ...
                    if operation == 'add':
                        if is_san_unique(san_number):
                            ...
                            workbook.save(workbook_path)  # Save after each SAN for immediate commitment
                            entered_sans_count += 1
                        else:
                            tk.messagebox.showerror("Error", "Duplicate or already used SAN number.", parent=root)
                    # Handle 'subtract' operation if necessary

            # After handling SAN inputs or if no SANs are needed, update the count
            for row in item_sheet.iter_rows(min_row=2):
                if row[0].value == selected_item:
                    row[1].value = row[2].value or 0  # Update LastCount to the current NewCount
                    # Update NewCount based on operation
                    if operation == 'add':
                    ...
                    workbook.save(workbook_path)  # Save workbook after updating counts
            
            if (san_required and entered_sans_count > 0) or not san_required:
                # Log the overall change for non-SAN items or after all SANs are handled
